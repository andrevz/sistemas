...
if (isset($_POST["MM_insert"]) && $insertar) {
   if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      if (!(isset($_GET["t"]) && $_GET["t"]=="popup")) {
         ...
         <script language='javascript'>cargaContenido(\"tiposgastos\"); </script>
        ";
      }
      exit;
   }
                ...
                <tr>
                    ...
                    <form method="POST" name="actualizar">
                      <table align="center">
                        ...
                        <tr valign="baseline">
                          <td colspan="2" align="center"><input class="boton1" type="submit" value="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "Editar"; } else { print "Crear"; }?> costo">
                          <input class="boton1" name="Cancelar" type="button" onClick="<?php if ((isset($_GET["t"]) && $_GET["t"]=="popup")) { print "window.close();"; } else { print "window.location='tiposgastos.php'"; } ?>" value='Cancelar'/></td>
                        </tr>
                      </table>
                      <input type="hidden" name="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "MM_edit"; } else { print "MM_insert"; } ?>" value="actualizar">
                      <?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "<input type='hidden' name='idtiposcostos' value='".$_GET["id"]."'>"; } ?>
                    </form>
<?php
    print $error;
?>
