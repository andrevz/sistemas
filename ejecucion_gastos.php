...
if (isset($_POST["MM_insert"]) && $insertar && !$bloqueado) {
    ...
    if ( $_POST["tipogastos"]=="-1" || $_POST["valor"]=="") {
      ...
      $sql="update ejecuciongastos set
                   nromes='".$_POST["nromes"]."',
          ...
          <table class='contenido' width='100%' id='equipo'>
                <tr>
                    <td colspan="7" class="titmenu">
                        EJECUCI&Oacute;N DE GASTOS<a name='nuevo'>
                    </td>
                </tr>                <tr>
                    <td class='tititems' width='90' >Nro. Mes</td>
                    <td class='tititems' width='320' >Concepto</td>
                    <td class='tititems' width='80' align='center'>Valor</td>
                ...
                <?php
                    ...
                    $col2="#ffffff";
                    if (isset($_GET["ids"])) {
                       $sql="SELECT * FROM ejecuciongastos pg inner join tiposgastos tg on tg.idtiposgastos=pg.idtiposgastos where idversion=".$_GET["idv"]." and idejecuciongastos<>".$_GET["ids"]." order by nromes";
                    } else {
                ...
                if ($fila[3]!=4) {
                   if ($eliminar && !$bloqueado) {
                      print "<a onclick='if(confirm(\"Esta seguro que desea eliminar este costo?\")) { return true;} else { return false; }' href='ejecucion_gastos.php?mode=d&ids=".$fila[0]."&idv=".$_GET["idv"]."'><img src=\"images/delete.png\" alt=\"Eliminar\" title=\"Eliminar\" width=\"18\" height=\"18\" border=\"0\" /></a>";
                   }
                   if ($editar && !$bloqueado) {
                      print "<a href='ejecucion_gastos.php?mode=e&ids=".$fila[0]."&idv=".$_GET["idv"]."#editor'><img src=\"images/editar.png\" alt=\"Editar\" title=\"Editar\" width=\"18\" height=\"18\" border=\"0\" /></a>";
                   }
                }
                print "</td>
                          </tr>";
          ...
          if (isset($_GET["ids"])) {
             ...
             <table>
                <tr>
                    ...
                    <td width='320'>
                    <select name='tipogastos' id='tipogastos' >
                           <option value='-1'>--- Elija ---</option>
                          <?php
                    ...
                    <?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { ?>
                          <input class="cancel" type="button" name="cancelar" size="60" value="" onclick="window.location='ejecucion_gastos.php?idv=<?php print $_GET["idv"]; ?>'">
                    <?php } ?>
                         </td>
                        </tr>
                      </table>
