...
class PDF_MemImage extends FPDF
{
    ...
    //    $this->SetFillColor(230,240,255);
        ...
        $this->Cell(50);
        $this->Cell(100,7,'REPORTE PLAN DE MEDIOS POR CIUDAD',0,1,'C');
        $this->SetFont('Arial','B',12);
...
$pdf->AddPage();
    ...
    $pdf->SetFont('Arial','',10);
     ...
     $sql_versiones="select idversion, gestion, c.nombre, inicioprogramado, nombres, apellidos, p.nombre, p.idprograma, c.idciudad, p.idescuela, p.idtipoprograma
                                 ...
                                 order by c.nombre, p.nombre, gestion";
      ...
      while ($fila_versiones=mysqli_fetch_array($res_versiones)) {
         if (acceso($_SESSION['idRol'], $fila_versiones[0],$fila_versiones["idescuela"],$fila_versiones["idtipoprograma"],$fila_versiones["idciudad"],0,0)>=2) {
            if ($ciudad_temp!=$fila_versiones[2]) {
               if ($i>0) {
                  $pdf->SetFont('Arial','B',7);
                  $pdf->Cell(148,5,"TOTALES ".$ciudad_temp,1,0,'L');
                  $pdf->Cell(17,5,sprintf("%0.2f",$total_planificado),1,0,'R');
                  $pdf->Cell(15,5,sprintf("%0.2f",$total_ejecutado),1,0,'R');
                  $pdf->Cell(12,5,sprintf("%0.2f",$total_saldo),1,1,'R');
                  $pdf->SetFont('Arial','',7);
               }

               $ciudad_temp=$fila_versiones[2];
               $i=0;
               $total_planificado=0;
               $total_ejecutado=0;
               $total_saldo=0;
               ...
               $pdf->Cell(192,5,"CIUDAD ".$ciudad_temp,1,1,'C');

               $pdf->Cell(7,5,"Nro",1,0,'L');
               $pdf->Cell(130,5,"Programa",1,0,'L');
               $pdf->Cell(11,5,"Version",1,0,'L');
//               $pdf->Cell(20,5,"Ciudad",1,0,'L');
               $pdf->Cell(17,5,"Monto planif.",1,0,'L');
               $pdf->Cell(15,5,"Monto ejec.",1,0,'L');
               $pdf->Cell(12,5,"Saldo",1,1,'L');
               $pdf->SetFont('Arial','',7);
            }
            ...
            $sql_pres="select presupuesto, gastos from
                    ...
                    $pdf->Cell(130,5,htmlspecialchars_decode ( substr($fila_versiones[6],0,85)),1,0,'L');
                    $pdf->Cell(11,5,$fila_versiones[1],1,0,'R');
//                    $pdf->Cell(20,5,$fila_versiones[2],1,0,'L');
                    $pdf->Cell(17,5,sprintf("%0.2f",$fila_pres[0]),1,0,'R');
                    $pdf->Cell(15,5,sprintf("%0.2f",$fila_pres[1]),1,0,'R');
                    $pdf->Cell(12,5,sprintf("%0.2f",$fila_pres[0]-$fila_pres[1]),1,1,'R');
