...
if (isset($_POST["MM_insert"])) {
    if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      if (!(isset($_GET["t"]) && $_GET["t"]=="popup")) {
         ...
         <script language='javascript'>cargaContenido(\"tipoconvocatoria\"); </script>";
      }
      exit;
   }
   ...
   ?>
    <td width="200" valign="top">
        <table align="center" width="100%">
                        ...
                        <tr>
                            <td class="textsubmenu"><img src="images/ok.gif" /></td><td class="textsubmenu"><a href="convocatorias.php" class="submenu">Editar clientes</a></td>
                        </tr>
        </table>
                ...
                <tr>
                    <td colspan="2" class="titmenu">
                        <?php if ($_GET["mode"]=='e') {
                              ...
                              ?> TIPO DE CONVOCATORIA<a name='nuevo'>
                    </td>
                </tr>
                <tr>
                    <td>
                    <form method="POST" name="actualizar">
                      <table align="center">
                        ...
                        <tr valign="baseline">
                          <td colspan="2" align="center"><input class="boton1" type="submit" value="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "Editar"; } else { print "Crear"; }?> tipo convocatoria">
                          <input class="boton1" name="Cancelar" type="button" onClick="<?php if ((isset($_GET["t"]) && $_GET["t"]=="popup")) { print "window.close();"; } else { print "window.location='convocatorias.php'"; } ?>" value='Cancelar'/></td>
                        </tr>
                      </table>
                      <input type="hidden" name="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "MM_edit"; } else { print "MM_insert"; } ?>" value="actualizar">
                      <?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "<input type='hidden' name='idtc' value='".$_GET["id"]."'>"; } ?>
                    </form>
<?php
    print $error;
?>
