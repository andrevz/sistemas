...
if (isset($_POST["MM_insert"])) {
    if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      $sql="insert into sector values (null, '".$_POST["nombre"]."')";
      ...
      if (!(isset($_GET["t"]) && $_GET["t"]=="popup")) {
         ...
         <script language='javascript'>cargaContenido(\"sector\"); </script>";
      }
      exit;
   }
    ...
    if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      $sql="update sector set nombre='".$_POST["nombre"]."' where idsector=".$_POST["ids"];
      mysql_query($sql) or die(mysql_error());
   ...
   ?>
    <td width="200" valign="top">
        <table align="center" width="100%">
                        ...
                        <tr>
                            <td class="textsubmenu"><img src="images/ok.gif" /></td><td class="textsubmenu"><a href="sectores.php" class="submenu">Editar Sectores</a></td>
                        </tr>
        </table>
            ...
            <table class="contenido" width="100%">
                <tr>
                    <td colspan="2" class="titmenu">
                        <?php if ($_GET["mode"]=='e') {
                                  print "EDITAR";
                                  $sqle="select * from sector where idsector=".$_GET["id"];
                ...
                <tr>
                    ...
                    <form method="POST" name="actualizar">
                      <table align="center">
                        ...
                        <tr valign="baseline">
                          <td colspan="2" align="center"><input class="boton1" type="submit" value="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "Editar"; } else { print "Crear"; }?> sector">
                          <input class="boton1" name="Cancelar" type="button" onClick="<?php if ((isset($_GET["t"]) && $_GET["t"]=="popup")) { print "window.close();"; } else { print "window.location='sectores.php'"; } ?>" value='Cancelar'/></td>
                        </tr>
                      </table>
                      <input type="hidden" name="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "MM_edit"; } else { print "MM_insert"; } ?>" value="actualizar">
                      <?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "<input type='hidden' name='ids' value='".$_GET["id"]."'>"; } ?>
                    </form>
<?php
    print $error;
?>
