...
if (isset($_POST["MM_insert"])) {
    if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      $sql="insert into tiporecurso values (null, '".$_POST["nombre"]."')";
      ...
      if (!(isset($_GET["t"]) && $_GET["t"]=="popup")) {
         ...
         <script language='javascript'>cargaContenido(\"tipo\"); </script>";
      }
      exit;
   }
    ...
    if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      $sql="update tiporecurso set nombre='".$_POST["nombre"]."' where idtipo=".$_POST["idt"];
      mysqli_query($sql) or die(mysqli_error());
   ...
   ?>
    <td width="200" valign="top">
        <table align="center" width="100%">
                        ...
                        <tr>
                            <td class="textsubmenu"><img src="images/ok.gif" /></td><td class="textsubmenu"><a href="tipos.php" class="submenu">Editar Tipos</a></td>
                        </tr>
        </table>
            ...
            <table class="contenido" width="100%">
                <tr>
                    <td colspan="2" class="titmenu">
                        <?php if ($_GET["mode"]=='e') {
                                  print "EDITAR";
                                  $sqle="select * from tipo where idtipo=".$_GET["id"];
                ...
                <tr>
                    ...
                    <form method="POST" name="actualizar">
                      <table align="center">
                        ...
                        <tr valign="baseline">
                          <td colspan="2" align="center"><input class="boton1" type="submit" value="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "Editar"; } else { print "Crear"; }?> tipo">
                          <input class="boton1" name="Cancelar" type="button" onClick="<?php if ((isset($_GET["t"]) && $_GET["t"]=="popup")) { print "window.close();"; } else { print "window.location='tipos.php'"; } ?>" value='Cancelar'/></td>
                        </tr>
                      </table>
                      <input type="hidden" name="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "MM_edit"; } else { print "MM_insert"; } ?>" value="actualizar">
                      <?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "<input type='hidden' name='idt' value='".$_GET["id"]."'>"; } ?>
                    </form>
<?php
    print $error;
?>
