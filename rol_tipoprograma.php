...
if (isset($_POST["id_escuela"]) && ($editar || $insertar)) {
      $sql_buscaactividad="select * from roltipoprograma where idrol=".$_POST["id_rol"]." and idtipoprograma=".$_POST["id_escuela"];
      $res_ba=mysql_query($sql_buscaactividad);
      if ($fila_ba=mysql_fetch_array($res_ba)) {
         if ($_POST["nivelacceso"]=="-1") {
             $sql_up="delete from roltipoprograma where idroltipoprograma=".$fila_ba[0];
         } else {
             $sql_up="update roltipoprograma set idnivel=".$_POST["nivelacceso"]." where idroltipoprograma=".$fila_ba[0];
         }
      } else {
         $sql_up="insert into roltipoprograma values (null, '".$_POST["id_rol"]."', '".$_POST["id_escuela"]."', '".$_POST["nivelacceso"]."')";
      }
...
<?php
                    ...
                    $sql_modulos = "select e.idtipoprograma, e.nombre, ar.idrol, ar.idnivel
                                   from tipoprograma e left join roltipoprograma ar on ar.idtipoprograma=e.idtipoprograma
                                   and ar.idrol=".$rol;

                    $result = mysql_query($sql_modulos);
                    print "<table width='100%' border=0 cellspacing=1 cellpadding=1>
                                  <tr>
                                      <td width='80%' class='content'><b>Tipo de programa</b></td>
                                      <td class='content'><b>Acceso</b></td></tr>";
                    $temp_modulo="";
                    while ($fila=mysql_fetch_array($result)) {
                          print "<form name='asigna_actividad' method='post'><tr><td class='tablatxt'>&rarr; ".$fila["nombre"]."</td>
                          <td align='center'><input type='hidden' name='id_rol' value='".$rol."'><input type='hidden' name='id_escuela' value='".$fila["idtipoprograma"]."'>";
