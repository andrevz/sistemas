...
if (isset($_POST["id_escuela"]) && ($editar || $insertar)) {
      $sql_buscaactividad="select * from rolescuela where idrol=".$_POST["id_rol"]." and idescuela=".$_POST["id_escuela"];
      $res_ba=mysql_query($sql_buscaactividad);
      if ($fila_ba=mysql_fetch_array($res_ba)) {
         if ($_POST["nivelacceso"]=="-1") {
             $sql_up="delete from rolescuela where idrolescuela=".$fila_ba[0];
         } else {
             $sql_up="update rolescuela set idnivel=".$_POST["nivelacceso"]." where idrolescuela=".$fila_ba[0];
         }
      } else {
         $sql_up="insert into rolescuela values (null, '".$_POST["id_rol"]."', '".$_POST["id_escuela"]."', '".$_POST["nivelacceso"]."')";
      }
...
<?php
                    ...
                    $sql_modulos = "select e.idescuela, e.nombre, ar.idrol, ar.idnivel
                                   from escuela e left join rolescuela ar on ar.idescuela=e.idescuela
                                   and ar.idrol=".$rol;
                    ...
                    while ($fila=mysql_fetch_array($result)) {
                          ...
                          <td align='center'><input type='hidden' name='id_rol' value='".$rol."'><input type='hidden' name='id_escuela' value='".$fila["idescuela"]."'>";
