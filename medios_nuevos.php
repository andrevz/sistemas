...
if (isset($_POST["MM_insert"]) && $insertar) {
   if ($_POST["nombre"]=="" or is_null($_POST["nombre"])) {
      ...
      if (!(isset($_GET["t"]) && $_GET["t"]=="popup")) {
         ...
         <script language='javascript'>cargaContenido(\"medio\"); </script>";
      }
      exit;
   }
            ...
            <table class="contenido" width="100%">
                ...
                <tr>
                    ...
                    <form method="POST" name="actualizar">
                      <table align="center">
                        <tr valign="baseline">
                          <td  class="tititems" align="right">Nombre medio:</td>
                          <td><input type="text" name="nombre" size="40" value="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print $filae[1]; } ?>"></td>
                        </tr>
                        <tr valign="baseline">
                          <td colspan="2" align="center"><input class="boton1" type="submit" value="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "Editar"; } else { print "Crear"; }?> medio">
                          <input class="boton1" name="Cancelar" type="button" onClick="<?php if ((isset($_GET["t"]) && $_GET["t"]=="popup")) { print "window.close();"; } else { print "window.location='medios.php'"; } ?>" value='Cancelar'/></td>
                        </tr>
                      </table>
                      <input type="hidden" name="<?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "MM_edit"; } else { print "MM_insert"; } ?>" value="actualizar">
                      <?php if (isset($_GET["mode"]) && $_GET["mode"]=="e") { print "<input type='hidden' name='idmedio' value='".$_GET["id"]."'>"; } ?>
                    </form>
<?php
    print $error;
?>
